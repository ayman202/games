...
function parseSystemRequirements(formData: FormData): string | null {
  const os = String(formData.get("sysOs") || "").trim();
  const cpu = String(formData.get("sysCpu") || "").trim();
  const gpu = String(formData.get("sysGpu") || "").trim();
  const ram = String(formData.get("sysRam") || "").trim();
  const storage = String(formData.get("sysStorage") || "").trim();
  const extraLabels = formData.getAll("sysExtraLabel") as string[];
  const extraValues = formData.getAll("sysExtraValue") as string[];
  const extra = extraLabels
    .map((label, i) => ({ label: label.trim(), value: (extraValues[i] || "").trim() }))
    .filter((e) => e.label && e.value);

  if (!os && !cpu && !gpu && !ram && !storage && extra.length === 0) return null;
  return JSON.stringify({ os, cpu, gpu, ram, storage, extra });
}
...
export async function createGame(formData: FormData) {
  ...
  const game = await prisma.game.create({
    data: {
      ...
      systemRequirements: parseSystemRequirements(formData),
