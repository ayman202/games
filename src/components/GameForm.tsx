...
export default function GameForm({ action, categories, initial, submitLabel }: Props) {
  ...
  return (
    <form action={action} className="flex flex-col gap-4 max-w-3xl">
      ...
      <div>
        <label className="block text-sm text-gray-400 mb-1">Trailer / video URL (YouTube link)</label>
        <input name="videoUrl" defaultValue={initial?.videoUrl} placeholder="https://youtube.com/watch?v=..." className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 outline-none focus:border-accent" />
      </div>
      ...
      <button className="bg-accent text-white rounded-lg py-2 font-semibold hover:opacity-90 mt-2">{submitLabel}</button>
    </form>
  );
}
